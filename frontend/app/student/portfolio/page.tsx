    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                setLoading(true);
                setErrorMsg(null);
                const [activeRes, activitiesRes, workingsRes, userRes] = await Promise.all([
                    // fetchMyPortfolios(),
                    fetchPortfolioByStatusActive(),
                    fetchActivities(),
                    fetchWorkings(),
                ...
                const activePortfolio = activeRes.data || activeRes;

                console.log("Active Portfolio form API:", activePortfolio);
                
                if (activePortfolio && activePortfolio.ID) {
                    // ตรวจสอบว่า ID ตรงกับ URL หรือไม่
                    // (กรณี User มี Active Portfolio แต่ URL ระบุ ID อื่น ก็ไม่ควรให้เข้า)
                    ...
                    // console.warn(`Mismatch: URL ID ${id} vs Active ID ${activePortfolio.ID}`);
                    setErrorMsg("ไม่พบแฟ้มสะสมผลงานที่เปิดใช้งาน (Active)");
                }
